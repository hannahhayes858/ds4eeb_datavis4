...
subtitle: "Jan. 14th, 2026, Hannah Hayes & Paige Gardner"
format: pdf
---

# Overview

Welcome! Today's activity is all about the basics of customizing plots. We will continue using `ggplot2` and will use a built-in dataset for part 1 and the old abalone data for part 2. For part 2, you will work with the same partners from last activity and modify the abalone graph you made together.
...
2.  A revised plot and your answers from the final abalone section. Submit that content to the same google slides as before and insert it into a slide below your previous slide. <https://docs.google.com/presentation/d/1wAGmbArAW2YKmufrBbzCQ3BcDdwDknzgoRHYEmFXJko/edit?usp=sharing>
...
Q1.1: It looks like there are three separate clusters forming on the plot. All with a positive correlation between bill depth and length.
...
ggplot(data = penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  ...
  scale_color_brewer(palette = "Dark2") +
  scale_shape_manual(values = c(21, 22, 24)) +
  scale_y_continuous(limits = c(0, NA)) # change the max y value to NA
...
# Q 1.6
?element_text
# the four options are "plain", "italic", "bold", "bold.italic"
```
...
Q2.1: We're presenting our figure to our grandma over family dinner. She has no background in science, but used to fish for abalone back in the day.
...
```{r}
abs <- read.csv("abalone_landings.csv")
abs
plot = ggplot(abs, aes(x = Year, y= Abalone_Landings_lbs))+
...
Q2.3: We would like to take the points off, change x and y axis labels, change the theme, and change the legend, to be clearer.
...
```{r}
ggplot(abs, aes(x = Year, y= Abalone_Landings_lbs))+
  geom_line(aes(color= Abalone_Species), linewidth = 1) + 
  labs(y = "Abalone Caught (lbs)",
       color="Abalone Species",
       title = "Abalone Caught 1973-1997") + 
  theme_classic() + 
...
We hope that grandma is able to take away how popular black abalone used to be, and that something happened in the 1970s that caused their populations to crash. Maybe even ask her if she remembers what happened that year that was different from years past that we could use to help determine what happened at that point and time.
